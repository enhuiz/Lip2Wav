...
def prepare_run(args):
    ...
    print("Training on {} hours".format(len(all_images) / (3600.0 * hparams.fps)))
    print(
        "Validating on {} hours".format(len(all_test_images) / (3600.0 * hparams.fps))
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--preset", help="Speaker-specific hyper-params", type=str, required=True
    )
    ...
    parser.add_argument(
        "--eval_interval",
        type=int,
        default=1000,  # Was 10000
        help="Steps between eval on test data",
    )
    parser.add_argument(
        "--tacotron_train_steps",
        type=int,
        default=2000000,  # Was 100000
