...
parser.add_argument(
    "-r", "--results_root", help="Path to test results folder", required=True
)
...
for filename in tqdm(all_files):
    ...
    if len(ref) > len(deg):
        ref = ref[0 : deg.shape[0]]
    elif len(deg) > len(ref):
        deg = deg[: ref.shape[0]]

    total_pesq += pesq(rate, ref, deg, "nb")
    total_stoi += stoi(ref, deg, rate, extended=False)
    total_estoi += stoi(ref, deg, rate, extended=True)
