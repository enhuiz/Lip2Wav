...
class Tacotron2:
    def __init__(self, checkpoint_path, hparams, gta=False, model_name="Tacotron"):
        ...
        split_infos = (
            tf.placeholder(
                tf.int32, shape=(hparams.tacotron_num_gpus, None), name="split_infos"
            ),
        )
        ...
        with tf.variable_scope("Tacotron_model") as scope:
            ...
            self.linear_outputs = (
                self.model.tower_linear_outputs
    ...
    def synthesize(
        self, texts, basenames, out_dir, log_dir, mel_filenames, embed_filenames
    ):
        ...
        for i in range(self._hparams.tacotron_num_gpus):
            ...
            input_seqs = (
                np.concatenate((input_seqs, device_input), axis=1)
        ...
        if self.gta:
            ...
            target_seqs = None
            for i in range(self._hparams.tacotron_num_gpus):
                device_target = np_targets[
                    size_per_device * i : size_per_device * (i + 1)
                ]
                device_target, max_target_len = self._prepare_targets(
                    device_target, self._hparams.outputs_per_step
                )
                target_seqs = (
                    np.concatenate((target_seqs, device_target), axis=1)
        ...
        for i, mel in enumerate(mels):
            ...
            if log_dir is not None:
                ...
                plot.plot_alignment(
                    alignments[i],
                    os.path.join(
                        log_dir, "plots/alignment-{}.png".format(basenames[i])
                    ),
                ...
                if hparams.predict_linear:
                    ...
                    audio.save_wav(
                        wav,
                        os.path.join(
                            log_dir, "wavs/wav-{}-linear.wav".format(basenames[i])
                        ),
                        sr=hparams.sample_rate,
                    )
                    ...
                    plot.plot_spectrogram(
                        linears[i],
                        os.path.join(
                            log_dir, "plots/linear-{}.png".format(basenames[i])
                        ),
                        title="{}".format(texts[i]),
                        split_title=True,
                        auto_aspect=True,
