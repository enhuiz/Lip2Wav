...
def add_train_stats(model, hparams):
    with tf.variable_scope("stats") as scope:
        ...
        if hparams.predict_linear:
            ...
            for i in range(hparams.tacotron_num_gpus):
                tf.summary.histogram(
                    "mel_outputs %d" % i, model.tower_linear_outputs[i]
                )
                tf.summary.histogram(
                    "mel_targets %d" % i, model.tower_linear_targets[i]
                )
        ...
        if hparams.tacotron_teacher_forcing_mode == "scheduled":
            tf.summary.scalar(
                "teacher_forcing_ratio", model.ratio
            )  # Control teacher forcing
        # ratio decay when mode = "scheduled"
        gradient_norms = [tf.norm(grad) for grad in model.gradients]
        tf.summary.histogram("gradient_norm", gradient_norms)
        tf.summary.scalar(
            "max_gradient_norm", tf.reduce_max(gradient_norms)
        )  # visualize
...
def add_eval_stats(
    ...
    values = [
        tf.Summary.Value(
            tag="Tacotron_eval_model/eval_stats/eval_before_loss",
            simple_value=before_loss,
        ),
        tf.Summary.Value(
            tag="Tacotron_eval_model/eval_stats/eval_after_loss",
            simple_value=after_loss,
        ),
        tf.Summary.Value(
            tag="Tacotron_eval_model/eval_stats/stop_token_loss",
            simple_value=stop_token_loss,
        ),
        tf.Summary.Value(
            tag="Tacotron_eval_model/eval_stats/eval_loss", simple_value=loss
        ),
    ]
    if linear_loss is not None:
        values.append(
            tf.Summary.Value(
                tag="Tacotron_eval_model/eval_stats/eval_linear_loss",
...
def model_test_mode(args, feeder, hparams, global_step):
    with tf.variable_scope("Tacotron_model", reuse=tf.AUTO_REUSE) as scope:
        ...
        model.initialize(
            ...
            feeder.eval_speaker_embeddings,
            feeder.eval_mel_targets,
            targets_lengths=feeder.eval_targets_lengths,
            global_step=global_step,
            is_training=False,
            is_evaluating=True,
...
def train(log_dir, args, hparams):
    ...
    log(
        "Tacotron training set to a maximum of {} steps".format(
    ...
    with tf.Session(config=config) as sess:
        try:
            ...
            if args.restore:
                ...
                try:
                    ...
                    if checkpoint_state and checkpoint_state.model_checkpoint_path:
                        ...
                        log(
                            "Loading checkpoint {}".format(hparams.eval_ckpt),
                            slack=True,
                        )
            ...
            while not coord.should_stop() and step < args.tacotron_train_steps:
                ...
                message = (
                    "Step {:7d} [{:.3f} sec/step, loss={:.5f}, avg_loss={:.5f}]".format(
                ...
                if (
                    ...
                    or step == args.tacotron_train_steps
                    or step == 300
                ):
                    ...
                    np.save(
                        os.path.join(mel_dir, mel_filename),
                        mel_prediction.T,
                        allow_pickle=False,
                    )
                    ...
                    plot.plot_spectrogram(
                        mel_prediction,
                        os.path.join(
                            plot_dir, "step-{}-mel-spectrogram.png".format(step)
                        ),
                        title="{}, {}, step={}, loss={:.5f}".format(
                            "Tacotron", time_string(), step, loss
                        ),
                ...
                if (
                    step % args.embedding_interval == 0
                    or step == args.tacotron_train_steps
                    or step == 1
                ):
            ...
            log(
                "Tacotron training complete after {} global steps!".format(
                    args.tacotron_train_steps
                ),
                slack=True,
            )
