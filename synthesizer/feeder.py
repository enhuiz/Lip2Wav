...
class Feeder:
    ...
    def __init__(self, coordinator, hparams):
        ...
        self.filelist = {
            "train": self._hparams.all_images,
        ...
        with tf.device("/cpu:0"):
            ...
            self._placeholders = [
                tf.placeholder(
                    tf.float32,
                    shape=(None, hparams.T, hparams.img_size, hparams.img_size, 3),
                    name="inputs",
                ),
                tf.placeholder(tf.int32, shape=(None,), name="input_lengths"),
                tf.placeholder(
                    tf.float32,
                    shape=(None, hparams.mel_step_size, hparams.num_mels),
                    name="mel_targets",
                ),
                ...
                # SV2TTS
                tf.placeholder(
                    tf.float32, shape=(None, 256), name="speaker_embeddings"
                ),
            ]
            ...
            queue = tf.FIFOQueue(
                8,
                [tf.float32, tf.int32, tf.float32, tf.int32, tf.int32, tf.float32],
                name="input_queue",
            )
            ...
            (
                self.eval_inputs,
                self.eval_input_lengths,
                self.eval_mel_targets,
                self.eval_targets_lengths,
                self.eval_split_infos,
                self.eval_speaker_embeddings,
            ) = eval_queue.dequeue()
    ...
    def _prepare_batch(self, batches, outputs_per_step):
        ...
        targets_lengths = np.asarray(
            [x[-1] for x in batches], dtype=np.int32
        )  # Used to mask loss
        ...
        for i in range(self._hparams.tacotron_num_gpus):
            ...
            inputs = (
                np.concatenate((inputs, input_cur_device), axis=1)
                if inputs is not None
                else input_cur_device
            )
            mel_target_cur_device, mel_target_max_len = self._prepare_targets(
                [x[1] for x in batch], outputs_per_step
            )
            mel_targets = (
                np.concatenate((mel_targets, mel_target_cur_device), axis=1)
                if mel_targets is not None
                else mel_target_cur_device
            )
    ...
    def _pad_input(self, x, length):
        return np.pad(
            x, (0, length - x.shape[0]), mode="constant", constant_values=self._pad
        )

    def _pad_target(self, t, length):
        return np.pad(
            t,
            [(0, length - t.shape[0]), (0, 0)],
            mode="constant",
    ...
    def get_window(self, center_frame):
        ...
        if self._hparams.T % 2:
            window_ids = range(
                center_id - self._hparams.T // 2, center_id + self._hparams.T // 2 + 1
            )
        else:
            window_ids = range(
                center_id - self._hparams.T // 2, center_id + self._hparams.T // 2
            )
    ...
    def crop_audio_window(self, spec, center_frame):
        ...
        total_num_frames = int(
            (spec.shape[0] * self._hparams.hop_size * self._hparams.fps)
            / self._hparams.sample_rate
        )
