...
def get_image_list(split, data_root):
    ...
    with open(os.path.join(data_root, "{}.txt".format(split))) as vidlist:
        for vid_id in vidlist:
            vid_id = vid_id.strip()
            filelist.extend(
                list(glob(os.path.join(data_root, "preprocessed", vid_id, "*/*.jpg")))
            )
    return filelist
...
def get_testlist(data_root):
    ...
    print(
        "{} hours is available for testing".format(
            len(test_images) / (sif.hparams.fps * 3600.0)
        )
    )
    test_vids = {}
